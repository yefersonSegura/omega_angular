...
function runRemoteList(channel: OmegaChannel, api: FacturaApi): void {
  void firstValueFrom(api.list())
    .then((result: FacturaListResult) => {
      if (result.ok) {
    ...
    .catch((error) => {
      console.error('[Omega][factura.agent][remoteList]', error);
      channel.emitNamed(FacturaWire.listFailure, { reason: 'Unexpected factura list error' });
    });
}
...
export function createFacturaAgent(channel: OmegaChannel, api: FacturaApi): OmegaAgent {
  return new OmegaAgent(
    channel,
    [new FacturaLoadListBehavior()],
    (reaction: OmegaAgentReaction) => {
      if (reaction.action === FacturaAgentAction.remoteList) {
        runRemoteList(channel, api);
      }
    },
    (error, context) => {
      console.error('[Omega][factura.agent]', { error, context });
    },
  );
}
